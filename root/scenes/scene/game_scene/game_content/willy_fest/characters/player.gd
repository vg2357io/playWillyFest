...
func _input(event: InputEvent) -> void:
	...
	if Input.is_action_just_pressed("jump"):
		anim_player.play("human_animations/human_jump")
		jumping = true
...
func _physics_process(delta: float) -> void:
	...
	if velocity.x < 0.2 && velocity.y < 0.2 && velocity.z < 0.2 && velocity.x > -0.2 && velocity.y > -0.2 && velocity.z > -0.2:
		anim_player.play("human_animations/human_idle")
	move_and_slide()
...
func _walk(delta: float) -> Vector3:
	...
	anim_player.play("human_animations/human_run")
	return walk_vel
